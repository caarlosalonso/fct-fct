...
function createAbreviatedNameEventListener() {
    ...
    form.getInput('ciclo-nombre').addEventListener('input', (event) => {
        const input = this.getValue();
        const acronimoInput = form.getInput('ciclo-acronimo');
        if (input && input.length > 0) {
            acronimoInput.setValue(
                input.split(' ')
...
function drawTable(ciclos, ciclosLectivos, grupos) {
    ...
    lastColumn.onclick = () => alert('Añadir ciclo lectivo');
...
function createCicloCell(ciclo, rowIdx) {
    ...
    const cellContent = document.createElement('div');
    cellContent.textContent = `${ciclo.nombre} (${ciclo.acronimo})`;
